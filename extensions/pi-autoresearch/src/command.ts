  ...
  pi.registerCommand('autoresearch', {
    ...
    handler: async (args, extCtx) => {
      ...
      if (command === 'off') {
        ...
        // Stop watching jsonl (will restart on resume)
        if (runtime.jsonlWatcher) {
          runtime.jsonlWatcher.close();
          runtime.jsonlWatcher = null;
        }

        // Remove widget/overlay immediately so dashboard doesn't linger
        ctx.clearSessionUi(extCtx);
        extCtx.ui.notify(
          "Autoresearch mode OFF — worktree preserved (use '/autoresearch' to resume)",
          'info'
        );
        return;
      }
      ...
      if (!runtime.worktreeDir) {
        ...
        if (worktreePath) {
          ...
          extCtx.ui.notify('Failed to create autoresearch worktree — isolation required', 'error');
          return;
        }
      }
