...
import { EXPERIMENT_MAX_LINES, EXPERIMENT_MAX_BYTES } from '../constants.js';
import {
  formatNum,
  formatElapsed,
  parseMetricLines,
...
export function registerRunExperiment(pi: ExtensionAPI, ctx: RunToolContext) {
  pi.registerTool({
    name: 'run_experiment',
    ...
    async execute(_toolCallId, params, signal, onUpdate, extCtx) {
      ...
      if (state.maxExperiments !== null) {
        ...
        if (allResultsCount >= state.maxExperiments) {
          return {
            content: [
              {
                type: 'text',
