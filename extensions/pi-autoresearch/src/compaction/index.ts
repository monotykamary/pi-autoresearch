...
import type { ExperimentResult, ExperimentState } from '../types/index.js';
import { isBetter } from '../utils/index.js';

const RECENT_RUN_LIMIT = 50;

type RunStatus = ExperimentResult['status'];
...
function sessionSection(state: ExperimentState): string {
...
function currentSegmentRuns(state: ExperimentState): ExperimentResult[] {
  return state.results.filter((run) => run.segment === state.currentSegment);
}

function runCountLine(runs: ExperimentResult[]): string {
...
function countByStatus(runs: ExperimentResult[]): StatusCounts {
...
function bestRun(runs: ExperimentResult[], direction: 'lower' | 'higher'): ExperimentResult | null {
...
function recentRunsSection(state: ExperimentState): string {
...
/** Baseline metric for a run = first run in the same segment across full state. */
function baselineFor(run: ExperimentResult, all: ExperimentResult[]): number | null {
  const sameSegment = all.find((other) => other.segment === run.segment);
  return sameSegment?.metric ?? null;
}

function formatRunLine(run: ExperimentResult, baseline: number | null): string {
...
function padStatus(status: ExperimentResult['status']): string {
...
function formatDescription(run: ExperimentResult): string {
  return run.description ? `desc: ${run.description}` : '';
}

function formatAsiFields(asi: ExperimentResult['asi']): string[] {
  if (!asi) return [];
  return [
